...
Usage:
    ...
    python run_all.py --strict <directory>
...
def run_all_checks(
    ...
    if not skip_chain:
        ...
        chain_result = verify_chain(
            artifacts,
            skip_hash_verify=skip_hash_verify,
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--skip-hash",
        action="store_true",
        help="Skip individual hash verification while still checking chain links",
    )
    parser.add_argument(
        "--strict",
        action="store_true",
    ...
    if args.strict and args.skip_hash:
        print("Error: --strict and --skip-hash are mutually exclusive", file=sys.stderr)
        return EXIT_USAGE_ERROR
    ...
    try:
        result = run_all_checks(
            ...
            skip_hash_verify=args.skip_hash,
        )
